...
function EditorPage() {
  ...
  /** Applies a plan generated by the AI builder to the canvas, then saves
   *  it immediately so the graph the model produced isn't lost if the tab
   *  is closed before a manual save. */
  const handleAiApply = useCallback(
    (
      draft: { name?: string; nodes: StoredNode[]; edges: StoredEdge[] },
      mode: "replace" | "merge",
    ) => {
      const nextName = draft.name ?? name;
      const nextStoredNodes: StoredNode[] =
        mode === "replace" ? draft.nodes : [...nodes.map(toStored), ...draft.nodes];
      const nextStoredEdges: StoredEdge[] =
        mode === "replace"
          ? draft.edges
          : [
              ...edges.map((e) => ({
      ...
      setNodes(nextStoredNodes.map((n) => ({ ...n, type: "flow", data: { ...n.data } })) as Node[]);
      setEdges(nextStoredEdges.map((e) => withEdgeExtras(e)) as Edge[]);
      setDirty(true);
      ...
      void save({
        data: {
          ...
          name: nextName,
          active,
          nodes: nextStoredNodes,
          edges: nextStoredEdges,
        } as never,
      })
        .then(() => {
          setDirty(false);
    ...
    [id, name, active, nodes, edges, setNodes, setEdges, withEdgeExtras, save, qc],
  );
