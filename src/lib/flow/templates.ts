...
export const TEMPLATES: Template[] = [
  ...
  },
  {
    ...
    description:
      "A chat- and webhook-driven router that classifies each request, then hands it to a dedicated coding, thinking, research or general agent — each wired to its own model and its own prompt.",
    category: "AI",
    steps: [
      {
        kind: "chatTrigger",
        ...
        pos: { x: 40, y: 260 },
        params: {
          greeting: "Ask anything — I route to the best specialist.",
          sample: '{ "message": "Research the latest on edge AI", "sessionId": "demo" }',
        },
      },
      {
        kind: "webhookTrigger",
        ...
        pos: { x: 40, y: 420 },
        params: { path: "ai-router", method: "POST", respond: "whenFinished", secret: "" },
      },
      {
        kind: "if",
        label: "Has message?",
        pos: { x: 320, y: 340 },
        params: { left: "{{ $json.message }}", op: "isTruthy", right: "" },
      },
      {
        kind: "aiAgent",
        label: "Router Agent (classifier)",
        pos: { x: 600, y: 340 },
        params: {
          systemPrompt:
            "You are a routing classifier, not an assistant. Read the user's message and reply with exactly one lowercase word: coding, thinking, research, or general. coding = writing/debugging/reviewing code. research = needs current/external information. thinking = complex reasoning, planning, analysis or writing with no coding or live-data need. general = everything else. Reply with only that single word, nothing else.",
      ...
      },
      {
        ...
        params: {
          field: "output",
          rules:
            '[{ "op": "contains", "value": "coding" }, { "op": "contains", "value": "thinking" }, { "op": "contains", "value": "research" }]',
          fallback: "branch 4",
        },
      },
      {
        ...
        pos: { x: 1160, y: 80 },
        params: {
          systemPrompt: "Route target: coding. Answer using the attached model's coding system prompt.",
          userPrompt: "{{ $json.message }}",
          maxSteps: 6,
          sessionKey: "{{ $json.sessionId }}",
        },
      },
      {
        ...
        params: {
          ...
          baseUrl: "",
          systemPrompt:
            "You are an expert software engineer. Write correct, minimal, well-explained code. Call out edge cases and trade-offs. Prefer showing runnable code over prose.",
          __config: { provider: "deepseekModel", style: "openai", baseUrl: "https://api.deepseek.com/chat/completions" },
        },
      },
      {
        ...
        pos: { x: 1160, y: 360 },
        params: {
          systemPrompt: "Route target: thinking. Answer using the attached model's reasoning system prompt.",
          userPrompt: "{{ $json.message }}",
          maxSteps: 6,
          sessionKey: "{{ $json.sessionId }}",
        },
      ...
      },
      {
        ...
        pos: { x: 1160, y: 640 },
        params: {
          systemPrompt: "Route target: research. Use the web research tool for anything current before answering.",
          userPrompt: "{{ $json.message }}",
          maxSteps: 8,
          sessionKey: "{{ $json.sessionId }}",
        },
      },
      {
        ...
        params: {
          ...
          baseUrl: "",
          systemPrompt:
            "You are a research assistant. Ground every claim in the search results you're given, cite what you used, and flag anything you couldn't verify.",
          __config: {
      ...
      },
      {
        ...
        pos: { x: 1160, y: 920 },
        params: {
          systemPrompt: "Route target: general. Answer helpfully and concisely.",
          userPrompt: "{{ $json.message }}",
          maxSteps: 4,
          sessionKey: "{{ $json.sessionId }}",
        },
    ...
    links: [
      ...
      { from: 4, to: 3, sourceHandle: "ai_languageModel", targetHandle: "ai_languageModel" },
      { from: 3, to: 5 },
      { from: 5, to: 6, sourceHandle: "out0" },
      { from: 5, to: 8, sourceHandle: "out1" },
      { from: 5, to: 10, sourceHandle: "out2" },
      { from: 5, to: 13, sourceHandle: "out3" },
      { from: 7, to: 6, sourceHandle: "ai_languageModel", targetHandle: "ai_languageModel" },
      { from: 9, to: 8, sourceHandle: "ai_languageModel", targetHandle: "ai_languageModel" },
      { from: 11, to: 10, sourceHandle: "ai_languageModel", targetHandle: "ai_languageModel" },
      { from: 12, to: 10, sourceHandle: "ai_tool", targetHandle: "ai_tool" },
      { from: 14, to: 13, sourceHandle: "ai_languageModel", targetHandle: "ai_languageModel" },
    ],
  },
];
